...
function firebaseNotificationsEnabled() {
  const raw = String(process.env.FIREBASE_NOTIFICATIONS_ENABLED || "true")
    .trim()
    .toLowerCase();
...
async function sendAdminPushNotification(title, body, data = {}) {
  if (!firebaseNotificationsEnabled()) {
    console.warn(
      "Skipping Firebase admin push notification; FIREBASE_NOTIFICATIONS_ENABLED is false."
    );
    return null;
  }
  ...
  try {
    ...
    for (const adminDoc of adminsSnapshot.docs) {
      ...
      tokensSnapshot.forEach((doc) => {
        if (doc.id) allTokens.push(doc.id);
      });
    }
    ...
    const message = {
      ...
      tokens: allTokens,
      data: toFcmData(data), // ✅ STRING-ONLY
    };
    ...
    if (response.failureCount > 0) {
      response.responses.forEach((resp, idx) => {
        if (!resp.success) {
          console.error(
            `Failed to send FCM to token ${allTokens[idx]}:`,
...
async function addAdminFirestoreNotification(
  ...
  if (!firebaseNotificationsEnabled()) {
    console.warn(
      "Skipping Firebase Firestore notification; FIREBASE_NOTIFICATIONS_ENABLED is false."
    );
    return null;
  }
