...
const { createEmailClient } = require("./services/emails");
...
const createAdminControlsModule = require('./routes/adminControls');
...
const { sendMailWithFallback } = createEmailClient();
...
async function updateOrderBoth(orderId, partialData = {}, options = {}) {
  ...
  const shouldUseExisting =
    options.existingOrder && typeof options.existingOrder === "object";
  const existingSnap = shouldUseExisting ? null : await orderRef.get();
  const existing = shouldUseExisting ? options.existingOrder : existingSnap.data() || {};
  const userId = options.userId ?? existing.userId;
  ...
  if (options.skipReload) {
    return { order: { id: orderId, ...existing, ...dataToMerge }, userId };
  }
...
const { router: adminControlsRouter, refreshKitTrackingById, refreshEmailLabelTrackingById } =
  createAdminControlsModule({
    admin,
    axios,
    ordersCollection,
    usersCollection,
