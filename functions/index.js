...
async function createShipEngineLabel(fromAddress, toAddress, labelReference, packageData, context = {}) {
  ...
  function assertUpsConfig({ carrierCode, carrierId }) {
    if (carrierCode !== "ups") {
      return;
    }
  ...
  assertUpsConfig({ carrierCode, carrierId: resolvedCarrierId });
  ...
  try {
    ...
    const normalizedMessage = String(messagePrefix || "").toLowerCase();
    if (carrierCode === "ups" && normalizedMessage.includes("missing or invalid shipper number")) {
      const upsConfigError = new Error(
        "UPS label creation failed: missing or invalid UPS shipper number/account configuration (verify carrier_id maps to the intended UPS account and remove mismatched manual billing/shipper settings)."
      );
      if (status) upsConfigError.status = status;
      upsConfigError.responseData = detail;
      upsConfigError.requestId = requestId;
      throw upsConfigError;
    }
